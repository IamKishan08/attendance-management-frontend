...
const Example = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://192.168.29.232:8080/api/attendance');
        ...
        if (selectedRows && selectedRows.rows && selectedRows.rows.length > 0) {
           try {
             ...
             for (const row of selectedRows.rows) {
               ...
               const response = await axios.delete(`http://192.168.29.232:8080/api/attendance/${employeeId}`);
  ...
  return(
    ...
    <Header />
    <div className="p-1">
        <h1 align="center" className="text-2xl font-bold mb-4">Attendance Report</h1>
